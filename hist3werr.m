...
if isempty(x)
    ...
    if all(~sigmanonzero) && ~half
        ...
        tic
        ncum=zeros(nbins);
        h1=fun{1}(histcEdges{1});
        h2=fun{2}(histcEdges{2});
        x1=fun{1}(x(:,1));
        x2=fun{2}(x(:,2));
        for c1=1:nbins(1)
            for c2=1:nbins(2)
                ncdf1=normcdfc(h1(c1+1),x1,sigmas(:,1));
                ncdf2=normcdfc(h2(c2+1),x2,sigmas(:,2));
                ncum(c1,c2)=sum(weights.*ncdf1.*ncdf2);
            end;
        end;
        toc
%
        tic
        ncdf1w=cell2mat(arrayfun(@(h) weights.*normcdfc(h,fun{1}(x(:,1)),sigmas(:,1)),fun{1}(histcEdges{1}(2:end)),'UniformOutput',0));
        ncdf2=cell2mat(arrayfun(@(h) normcdfc(h,fun{2}(x(:,2)),sigmas(:,2)),fun{2}(histcEdges{2}(2:end)),'UniformOutput',0));
        nadd=bsxfun(@times,ncdf1w,permute(ncdf2,[1,3,2]));
        ncum=squeeze(sum(nadd,1));
        clear ncdf1w ncdf2 nadd;
        toc
%
        tic
        ncum=squeeze(sum(bsxfun(@times, ...
            cell2mat(arrayfun(@(h) weights.*normcdfc(h,fun{1}(x(:,1)),sigmas(:,1)),fun{1}(histcEdges{1}(2:end)),'UniformOutput',0)),...
            permute(cell2mat(arrayfun(@(h) normcdfc(h,fun{2}(x(:,2)),sigmas(:,2)),fun{2}(histcEdges{2}(2:end)),'UniformOutput',0)),[1,3,2])),1));
        toc
%
        tic
        ncdf1w=repmat(weights,1,nbins(1)).*normcdfc(repmat(fun{1}(histcEdges{1}(2:end)),nrows,1),repmat(fun{1}(x(:,1)),1,nbins(1)),repmat(sigmas(:,1),1,nbins(1)));
        ncdf2=normcdfc(repmat(fun{2}(histcEdges{2}(2:end)),nrows,1),repmat(fun{2}(x(:,2)),1,nbins(2)),repmat(sigmas(:,2),1,nbins(2)));
        nadd=bsxfun(@times,ncdf1w,permute(ncdf2,[1,3,2]));
        ncum=squeeze(sum(nadd,1));
        clear ncdf1w ncdf2 nadd;
        toc
